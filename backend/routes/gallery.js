...
router.get('/', async (_req, res) => {
  const sections = await GallerySection.find({ isActive: true }).sort({ order: 1, createdAt: 1 })
  const sectionIds = sections.map((s) => s._id)
  const photos = await GalleryPhoto.find({ section: { $in: sectionIds } }).sort({ order: 1, createdAt: 1 })
...
router.delete('/sections/:id', protect, adminOnly, async (req, res) => {
  const section = await GallerySection.findById(req.params.id)
  if (!section) return res.status(404).json({ message: 'Section not found' })
...
router.post('/sections/:id/photos', protect, adminOnly, async (req, res) => {
  const section = await GallerySection.findById(req.params.id)
  if (!section) return res.status(404).json({ message: 'Section not found' })
...
// Admin: delete photo
router.delete('/photos/:id', protect, adminOnly, async (req, res) => {
